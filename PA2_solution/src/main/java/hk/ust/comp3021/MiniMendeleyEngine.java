        ...
        switch (action.getKind()) {
            case ID:
                paperBase.entrySet().forEach(entry -> {
                    if (action.isEqual.test(entry.getKey())) action.appendToActionResultByLambda.accept(entry.getValue());
                });
                break;
            case TITLE:
                paperBase.entrySet().forEach(entry -> {
                    if (action.isEqual.test(entry.getValue().getTitle())) action.appendToActionResultByLambda.accept(entry.getValue());
                });
                break;
            case AUTHOR:
                paperBase.entrySet().forEach(entry -> {
                    if (action.isContain.test(entry.getValue().getAuthors())) action.appendToActionResultByLambda.accept(entry.getValue());
                });
                break;
            case JOURNAL:
                paperBase.entrySet().forEach(entry -> {
                    if (action.isEqual.test(entry.getValue().getJournal())) action.appendToActionResultByLambda.accept(entry.getValue());
                });
                break;
            default:
                break;
    ...
    public List<Paper> processSortPaperActionByLambda(User curUser, SortPaperAction action) {
        ...
        switch (action.getBase()) {
            case ID:
                ...
                if (action.kindPredicate.test(action.getKind())) action.comparator = action.comparator.reversed();
                break;
            case TITLE:
